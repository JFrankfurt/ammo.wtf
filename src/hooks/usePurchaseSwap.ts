...
import { useUniswapSwap } from "./useSwap"; // Assuming this is the correct path
...
export function usePurchaseSwap({
  ...
  const { state: swapState, swap } = useSwap(chainId);
  ...
  useEffect(() => {
    logger.debug("Success propagation effect triggered", {
    ...
    if (swapState.txHash && onSuccess) {
      ...
      logger.debug(
        "Transaction successful (txHash present), but no onSuccess callback provided."
      );
    }
  ...
  useEffect(() => {
    logger.debug("Error propagation effect triggered", {
    ...
    if (swapState.error && onError) {
      ...
      logger.debug(
        "Calling onError callback with swapState error:",
        swapState.error
      );
      onError(swapState.error);
    } else if (swapState.error) {
      logger.debug(
        "Swap error occurred (swapState.error present), but no onError callback provided."
      );
    }
  ...
  const executeSwap = async () => {
    ...
    try {
      logger.debug(
        `Executing swap: ${totalCostUSDC} USDC (${usdcAddress}) for ${tokenOutAddress}`,
        {
      ...
      logger.debug(
        "Swap execution successful (underlying hook will update state)."
      );
    } catch (err) {
      ...
      if (onError && err instanceof Error && !swapState.error) {
        ...
        logger.debug(
          "Swap execution error occurred, but no onError callback provided or error already in swapState."
        );
      } else {
