...
export async function decryptSymmetricKey(
  ...
  const sharedKey = new Uint8Array(sharedSecret).slice(
    0,
    encryptedSymmetricKey.length
  );
  const symmetricKeyBytes = encryptedSymmetricKey.map(
    (byte, i) => byte ^ sharedKey[i]
  );
...
export async function example() {
  ...
  console.log(
    "Encrypted data posted to blockchain:",
    blockchain.shippingTransaction
  );

  // Shipper retrieves and decrypts the data
  const retrievedData = blockchain.shippingTransaction;
  const decryptedAesKey = await decryptSymmetricKey(
    retrievedData,
    shipperPublicKey
  );
  const decryptedShippingInfo = await decryptData(
